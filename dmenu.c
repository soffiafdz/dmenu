...
usage(void)
{
	fputs("usage: dmenu [-bfiv] [-l lines] [-h height] [-p prompt] [-fn font]\n"
	      "             [-nb color] [-nf color] [-sb color] [-sf color] [-m monitor]\n"
...
readxresources(void) {
	...
	if ((xrm = XResourceManagerString(drw->dpy))) {
		...
		if (XrmGetResource(xdb, "dmenu.background", "*", &type, &xval) ||
			  XrmGetResource(xdb, "dmenu.color0", "*", &type, &xval))
			colors[SchemeNorm][ColBg] = strdup(xval.addr);
		else
			colors[SchemeNorm][ColBg] = strdup(colors[SchemeNorm][ColBg]);
		if (XrmGetResource(xdb, "dmenu.foreground", "*", &type, &xval) ||
			  XrmGetResource(xdb, "dmenu.color15", "*", &type, &xval))
		...
		if (XrmGetResource(xdb, "dmenu.nhiforeground", "*", &type, &xval) ||
			  XrmGetResource(xdb, "dmenu.color3", "*", &type, &xval))
		...
		if (XrmGetResource(xdb, "dmenu.hipforeground", "*", &type, &xval) ||
			  XrmGetResource(xdb, "dmenu.color1", "*", &type, &xval))
